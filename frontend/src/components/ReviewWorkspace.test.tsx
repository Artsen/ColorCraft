...
describe('ReviewWorkspace outcomes', () => {
  it('renders the wheel beside a concise measured overview', () => {
    ...
    expect(screen.getByText('This measures geometric color relationships, not subjective design quality.')).toBeInTheDocument()
  })
  ...
  it('orders relationships by confidence and qualifies weak matches', () => {
    ...
    expect(relationships.map((item) => item.type)).toEqual(['complementary', 'analogous'])
    renderReview('harmony')
    const cards = document.querySelectorAll('.relationship-card')
    expect(within(cards[0] as HTMLElement).getByText(/Strong complementary relationship/)).toBeInTheDocument()
    expect(within(cards[1] as HTMLElement).getByText(/Weak analogous relationship/)).toBeInTheDocument()
    expect(screen.getByText('Two colors are near the edge of the configured analogous range.')).toBeInTheDocument()
  })
  ...
  it('supports role assignment, contrast calculation, previews, and advanced pairs', () => {
    ...
    fireEvent.change(screen.getByLabelText('Surface'), { target: { value: red.hex } })
    expect(onAssignRole).toHaveBeenCalledWith('surface', red.hex)
    expect(screen.getByText('Advanced: all-pairs contrast matrix')).toBeInTheDocument()
    view.unmount()
  })
